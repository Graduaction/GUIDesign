...
using NPOI.HSSF.UserModel; //HSSF适用于07年之前的xls版本
using NPOI.XSSF.UserModel;//XSSF适用于07年之后的xlsx版本
...
namespace BLL
{
     class ExcelHelper
    {
        ...
        //------------------------------------------------
        public static bool ReadFromExcel(string filePath, DataGridView dataGridView)//从excel中读取数据
        {
            ...
            try
            {
                ...
                workbook = new XSSFWorkbook(fs);//.xls
                if (fs != null)
                {
                    fs.Close();
                    fs.Dispose();
                ...
                rowCount = sheet.LastRowNum;//行数
                colCount = sheet.GetRow(0).LastCellNum;//列数
